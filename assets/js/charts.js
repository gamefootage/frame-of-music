...
function getCountryChart(countryCode) {
    $.ajax({
        url: `https://api.musixmatch.com/ws/1.1/chart.tracks.get?format=jsonp&callback=chartsJsonpCallback&page=1&page_size=21&country=${countryCode}&apikey=${apiKey}`,
        type: "GET",
        dataType: "jsonp",
...
function chartsJsonpCallback(json) {
    ...
    trackList.forEach(function(item, index) {
        if (index <= 9) {
            $("#charts-results-table-1 tbody").append(
                `<tr>
                    <td>${index + 1}</td> 
                    <td>
            ...
            $("#charts-results-table-2 tbody").append(
                `<tr>
                    <td>${index + 1}</td> 
                    <td>
                        <a class="text-decoration-none" target="_blank" href="${item.track.track_share_url}" alt="${item.track.track_name} Share Link">
